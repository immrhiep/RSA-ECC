...
''' Khai báo trước một số đường cong theo tiêu chuẩn của FIPS 186-3
Nhằm đảm bảo tính an toàn khi mã hoá dữ liệu
Bộ từ điển DOMAINS bao gồm Kích thước(bits), p, n, b, G(x, y)
Với y^2 = x^2 -3x + b mod p'''
...
def getCurve(bits=256):
    "Chọn đường cong Elliptic theo kích thước bits"

    if bits in DOMAINS:
        a = -3  # a theo tiêu chuẩn của FIPS
        p, n, b, x, y = DOMAINS[bits]
        return p, n, a, b, (x, y)
    else:
        # Báo lỗi nếu kích thước khoá không tồn tại trong Domain.
        raise KeyError, "Kích thước khoá %s không tồn tại trong Domains!" % bits
